...
class MyChat(basic.LineReceiver):
    ...
    def delUserFromClientMap(self, user):
        ...
        for key in self.factory.clientmap.keys():
            #print_trace("@@@@@ CA: looping keys user=", repr(user), '\n')
            if key == user: continue
            if key == self: continue
            #print_trace("@@@@@ CA: found another user")
            if isinstance(key, MyChat):
                #print_trace("@@@@@ CA: sending message")
    ...
    def dataReceived(self, line):
        ...
        if u: # send hello back to user and an announcement to everyone else.
            ...
            for key in self.factory.clientmap.keys():
                #print_trace ("@@@@@ CA: looping keys user=", repr(key))
                if key == user: continue
                if key == self: continue
                #print_trace("@@@@@ CA: found another user", repr(key))
                if isinstance(key, MyChat):
                    to_user = self.factory.clientmap[key]
                    #print_trace("@@@@@ CA: sending message to ", to_user)
